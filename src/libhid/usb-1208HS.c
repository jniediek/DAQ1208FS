...
void usbBuildGainTable_USB1208HS(usb_dev_handle *udev, float table[NMODE][NGAINS][2])
{
  /*
...
void usbBuildGainTable_USB1208HS_4AO(usb_dev_handle *udev, float table_AO[NCHAN_AO][2])
{
  /*
    Builds a lookup table of calibration coefficents to translate values into voltages:
...
__u16  usbDTristateR_USB1208HS(usb_dev_handle *udev)
{
  ...
  */
  ...
  if (usb_control_msg(udev, requesttype, DTRISTATE, 0x0, 0x0, (char *) &data, sizeof(data), HS_DELAY) < 0) {
    printf("usbDTristateR_USB1208HS: error in usb_control_msg().\n");
  }
  return data;
}
...
__u16 usbDPort_USB1208HS(usb_dev_handle *udev)
{
  ...
  if (usb_control_msg(udev, requesttype, DPORT, 0x0, 0x0, (char*) &data, sizeof(data), HS_DELAY) < 0) {
    printf("usbDPort_USB1208HS: error in usb_control_msg().\n");
  }
  return data;
}
...
__u16 usbDLatchR_USB1208HS(usb_dev_handle *udev)
{
  ...
  */
  ...
  if (usb_control_msg(udev, requesttype, DLATCH, 0x0, 0x0, (char *) &data, sizeof(data), HS_DELAY) < 0) {
    printf("usbDLatchR_USB1208HS: error in usb_control_msg().\n");
  }
  return data;
}
...
void usbDLatchW_USB1208HS(usb_dev_handle *udev, __u16 data)
{
  ...
  if (usb_control_msg(udev, requesttype, DLATCH, data, 0x0, NULL, 0x0, HS_DELAY) < 0) {
    printf("usbDLatchW_USB1208HS: error in usb_control_msg().\n");
  }
  return;
}
...
int usbAInScanRead(usb_dev_handle *udev, int nScan, int nChan, __u16 *data)
{
  int ret = -1;
  int remainder;
  int bRead = 0;
...
void usbAInConfig_USB1208HS(usb_dev_handle *udev, __u8 mode, __u8 range[NCHAN])
{
  /*
...
void usbAInConfigR_USB1208HS(usb_dev_handle *udev, __u8 *mode, __u8 range[NCHAN])
{
  struct t_AInConfig {
    __u8 mode;
    __u8 range[NCHAN];
  ...
  usb_control_msg(udev, requesttype, AIN_CONFIG, 0x0, 0x0, (char *) &AInConfig, sizeof(AInConfig), HS_DELAY);
  *mode = AInConfig.mode;
  memcpy(range, AInConfig.range, NCHAN);
}
 ...
 *            Analog Output                    *
 ***********************************************/
void usbAOut_USB1208HS(usb_dev_handle *udev, __u8 channel, double voltage, float table_AO[NCHAN_AO][2])
{
  /*
...
void usbAOutR_USB1208HS(usb_dev_handle *udev, __u8 channel, double *voltage, float table_AO[NCHAN_AO][2])
{
  __u16 value[4];
  __u8 requesttype = (DEVICE_TO_HOST | VENDOR_TYPE | DEVICE_RECIPIENT);
...
__u16 voltsTou12_USB1208HS_AO(double volts, int channel, float table_AO[NCHAN_AO][2])
{
    double dvalue;
  __u16 value;

...
{
  /*
    Burst I/O mode will sample data to the onboard SRAM FIFO until full, and then return
    the data in continuous messages using all 5 endpoints.  Prescaler values above 1:8 are
    not allowed in burst I/O mode.  Single execution and immediate transfer bits will be
    ignored in this mode.

    Immediate transfer mode is used for low sampling rates to avoid delays in receiving the
    sampled data.  The data will be sent at the end of every timer period, rather than
    waiting for the buffer to fill.  All 5 endpoints will still be used in a sequential manner.
    This mode should not be used if the aggregate sampling rate is greater then 32,000 samples
    per second in order to avoid data loss.

    The external trigger may be used to start data collection synchronously.  If the bit is
    set, the device will wait until the appropriate trigger edge is detected, then begin
    sampling data the specified rate.  No messages will be sent until the trigger is detected.

    External sync may be used to synchronize the sampling of multiple PMD-1608FS devices, or
    to sample data using an external clock.  The device must be set to be a sync slave with the
    SetSync command prior to using this mode.  Data will be acquired on all specified channels
    when the sync edge is detected.
   */
  int num_samples;
  int i, j, k;
  int ret;
  ...
  *frequency = 10.e6/(preload*(1<<arg.prescale));
  ...
  while ( num_samples > 0 ) {         
    for ( j = 1; j <= 6; j++ ) {         // cycle through the ADC interfaces/endpoints
      ret = usb_interrupt_read(hid[j]->dev_handle, USB_ENDPOINT_IN | (j+2), (char *) &data, sizeof(data), FS_DELAY);
      if (ret <= 0) {
	printf("AInScan: error reading pipe %d.  return code = %d\n", j, ret);
	return -1;
      }
      i = data.scan_index*31;  // data can come in any order in packets of [ 62 bytes data | 1 byte index ]
	...
	return count;
      ...
      printf("Scan index = %d\tPipe number = %d\tnumber samples left = %d\n", data.scan_index, j, num_samples);
    }
  }
  return count;
}

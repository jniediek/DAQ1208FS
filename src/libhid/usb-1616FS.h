...
#define NCHAN  16               // max number of A/D channels in the device
#define NGAINS 4                // max number of gain levels
...
signed short usbAIn_USB1616FS(HIDInterface* hid, __u8 channel, __u8 range, float table[NGAINS][NCHAN][2]);
...
int usbAInScan_USB1616FS(HIDInterface* hid[], __u8 lowchannel, __u8 highchannel, __u32 nScan,
			 float *frequency, __u8 options, __s16 sdata[], float table_AIN[NGAINS][NCHAN][2], __u8 gainArray[]);
...
void usbBuildCalTable_USB1616FS(HIDInterface* hid, float table[NGAINS][NCHAN][2]);
